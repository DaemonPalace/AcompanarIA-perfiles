...
Status as of 2026-09-03 (fourth round). See "Round 4" for Stage 2 (schema-wide real-data calibration), and "Round 3" for Stage 1 (ECOG Bayesian Network) and the clinical-consistency verification pipeline it produced. Everything below "Round 3" describes state as of 2026-08-10 (second round) and is otherwise unchanged.

## Round 4 (2026-09-03) — Stage 2: schema-wide real-data calibration

Full spec: `requirements.md` §6 (revised scope — see below). Approved same-day by Santiago, implemented same-day.

**Scope correction made before implementing, not after:** requirements.md originally described Stage 2 as calibrating "the full oncology-relevant portion of the schema." Checking actual TCGA field coverage first (`data/tcga_parsed/baseline_profile.csv`) showed this wasn't achievable — TCGA has zero ground truth for `functional`, `symptom`, `side_effect`, and non-psychiatric `medication` nodes (it's a genomics registry, not a symptom-tracking one). Revised scope, presented to and approved by the user before any implementation: recalibrate what TCGA actually measures (Tier A/B below), tag everything else honestly rather than pretend to calibrate it (Tier C).

**Tier A — real-data baseline recalibration** (`disease_stage`, `age_bracket`, `gender`, and a renamed `chemotherapy_current`→`has_chemo`):
- `disease_stage`: real TCGA distribution is Advanced 66.0% / Terminal 33.3% / Mild 0.45% / Moderate 0.29% (94% coverage), vs. the old literature-estimated 25/35/25/15 split — TCGA's heavy late-stage skew is appropriate for a palliative population.
- `age_bracket`: recalibrated to the real age distribution (99% coverage): 18-35 2.87% / 36-55 24.91% / 56-70 42.03% / 71+ 30.19%.
- `gender`: **only the Female/Male portion rescaled** to TCGA's real sex-at-birth ratio (57.8% Male / 42.2% Female) — TCGA cannot inform gender identity at all, so `Non-binary`/`Prefer not to say` keep their prior literature-estimated share (2%/1%) untouched, per explicit user decision.
- `chemotherapy_current` → **renamed to `has_chemo`**: TCGA's field means "ever received chemotherapy," not "currently receiving" — rather than keep a misleading name with a caveat, the node id, label, desc, and its two edges (`e_chemotherapy_current_fatigue`/`e_chemotherapy_current_nausea_vomiting` → `e_has_chemo_*`) were renamed to honestly match what TCGA measures, per explicit user decision. Baseline recalibrated to the real 63.7%/36.3% No/Yes split (close to the old 60/40 — mostly a truthfulness fix, not a big numeric shift).
- **`cancer_type_*` (7 site nodes) deliberately NOT touched**, per explicit user decision: TCGA's project mix in this parse has 0% hematologic and 0.1% gynecological representation — a sampling artifact of which studies got pulled into `context/data.json`, not real palliative-population epidemiology. The literature-curated distribution (8 dedicated palliative-cohort studies) stays authoritative; tagged `literature`, not `real_data`.

**Tier B — new Bayesian Network** (`training/train_metastatic_bn.py`, same `model_ref` mechanism as Stage 1's ECOG model): `metastatic_disease ← disease_stage`, trained on TCGA's `ajcc_pathologic_m` (1,770 usable cases after excluding `MX`/missing, 87% coverage). Real metastatic rate by stage: Mild/Moderate ~0%, Advanced 0.4%, Terminal 59.5% — replaces the old compound-formula gate (`target='No' if source<=1 else target`, which only enforced the Stage 1/2 exclusion and left Stage 3/Terminal at an ungrounded 55% baseline draw). **Much stronger result than Stage 1's ECOG model**: 87.0% accuracy / 0.823 macro-F1 vs. an 81.9% majority-baseline (Stage 1's ECOG model only tied its baseline) — a genuinely predictive relationship, not just an honest-but-weak one. Full report: `reports/stage2_metastatic_bn_eval.md`.

**Tier C — full-schema `evidence_tier` backfill**, the item explicitly deferred at the end of Stage 1. Every node and edge now carries a tag: `real_data` (6 nodes: the 4 Tier A + `metastatic_disease` + `ecog_performance_status` from Stage 1), `literature` (61 edges / 20 nodes — has a real citation: a named study, DSM-5, or the WHO analgesic ladder), or `clinical_estimate` (29 edges / 30 nodes — hand-set or this project's own fitted formulas, no external citation). Classification was mostly automated (keyword-matched against each edge's `ref` field, node tier inferred from its literature-tier inbound edges) with two manual overrides for edges that self-described as "clinical estimate...consistent with DSM-5" rather than directly citing a DSM-5 criterion.

**`generator/privacy.py` extended with the real DCR audit**, unblocked now that the real-data field space is large enough to be meaningful (Stage 1 alone was only `disease_stage × age_bracket`, 16 combinations — too small, would flag nearly every row as "matching" a real one by construction). **Found and fixed a real methodological bug while building it**: a naive 6-field DCR combo (including `ecog_performance_status`, only 19% reference coverage) gave a 27.1% "rare combination match" rate on real generated data — alarming, until a control test (resampling directly *from* the reference set — a trivially-safe "generator") also produced an 8.2% rate, proving the metric itself was inflated by missing-data fragmentation, not a real signal. Fixed by auto-excluding real_data fields below 80% reference coverage from the DCR combo (`min_field_coverage` parameter, self-updating — no hardcoded field list). Post-fix: control test ~1.25% (near-zero, as expected), real generated sample ~0.9%-1.1%, no false warnings. Wired into `server.py`'s `/api/analyze` as a new `"dcr"` key; gracefully reports "not available" when `data/` isn't present (it's gitignored, won't exist in the deployed container) rather than crashing.

**Stage 3 (clinical consistency verification) run against this round's output, per the mandatory gate**: `generator/clinical_consistency.py` clean (0 findings); LLM reviewer cross-tabs checked specifically for Stage-2-introduced issues (chronic_pain-vs-metastatic, depression-vs-stage, MMSE-vs-age, fatigue/nausea-vs-chemo, male-breast-cancer-rate regression check, overall functional/psych stability) — all clean, one apparent MMSE-vs-age non-monotonicity at n=8,000 traced to small-sample noise (the 18-35 bracket is now only ~2.9% of the population) and confirmed correctly monotonic at n=30,000. Full report: `reports/stage3_consistency_pass.md`.

**Verified:** schema validates; full HTTP round-trip (`/api/generate` CSV, `/api/analyze` with all four audit keys) on both the native server and a freshly-rebuilt Docker image (confirmed the DCR audit degrades gracefully — no `data/` in the container, no crash); schema bumped to v1.4.0.

**SEER — on standby, not used this round.** Access requested, not yet granted as of 2026-09-03. Once available, it's the natural source to revisit the `cancer_type_*` decision above (a population registry, not a curated research cohort — wouldn't share TCGA's zero-hematologic sampling gap) as a follow-up round, not a blocker to this one.
